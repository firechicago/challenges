...
SELECT title, year, rating FROM movies WHERE year < 1990 AND year > 1979
  AND genre_id = 17 ORDER BY rating DESC;
...
SELECT movies.title, movies.year, genres.name FROM movies JOIN cast_members ON
  movies.id = cast_members.movie_id JOIN actors ON cast_members.actor_id =
  actors.id JOIN genres ON movies.genre_id = genres.id WHERE actors.name =
  'Julianne Moore' ORDER BY genres.name, movies.title;
...
SELECT movies.title, movies.year, studios.name AS studio FROM movies JOIN genres ON
  movies.genre_id = genres.id LEFT JOIN studios ON movies.studio_id = studios.id
  WHERE genres.name = 'Horror' ORDER BY movies.year ASC LIMIT 5;
